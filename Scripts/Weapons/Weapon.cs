...
public partial class Weapon : Node3D
{
    ...
    void Initialize()
    {
        if (GetChild(4) is Node3D bulletHole && bulletHole != null)
        {
            BulletHole = bulletHole;

        }
        else GD.PrintErr("BulletHole is null!");
        if (GetChild(5) is RayCast3D rayCast3DAim && rayCast3DAim != null) RayCast3DAim = rayCast3DAim; else GD.PrintErr("RayCast Aim is null!");
        if (GetChild(7) is Node3D cassingSpawner && cassingSpawner != null) CassingSpawner = cassingSpawner; else GD.PrintErr("Cassing Spawner is null!");
        if (GetChild(8) is GpuParticles3D effect && effect != null) Effect = effect; else GD.PrintErr("Effect is null!");
