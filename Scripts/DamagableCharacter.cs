...
public abstract partial class DamagableCharacter : CharacterBody3D
{
    const float DropSoundLimit = 30f;

    [ExportGroup("Standard Mechanics")]
    [Export] public float Health = 100;
    [Export] public float Armour = 0;
    [ExportGroup("Fall Mechanics")]
    [Export] public int FallDamageLimit = 150;
    [Export] public int FallDamageAmount = 5;

    [ExportGroup("Step Mechanics")]
    [Export] public new float FloorMaxAngle = 45f;
    [Export] public float MaxStepHeight = 0.5f;
    [Export] public RayCast3D StairBelowRaycast;
    [Export] public RayCast3D StairAheadRaycast;

    [ExportGroup("Fall Sound")] // remember to initialize this 
    public AudioHandler DropSound;
    [Export] public string DropSoundPath = "res://Sounds/FallSound.mp3";


    public Vector3 velocity;
    public Damagable DamageHandler = new();
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        velocity.X = Mathf.Lerp(velocity.X, 0, (float)delta * 14.0f);
        velocity.Z = Mathf.Lerp(velocity.Z, 0, (float)delta * 14.0f);
        velocity += GetGravity() * (float)delta;
        Velocity = velocity;
        MoveAndSlide();
        SnapDownToStair();
    }
    public void InitializeDamagableCharacter()
    {
        DamageHandler.Health = Health;
        DamageHandler.Armour = Armour;

        DropSound = new AudioHandler(DropSoundPath);
        AddChild(DropSound);

    }
    public void CheckFallDamage()
    {
        if (!IsOnFloor() && Velocity.Y < 0) // prevents upward velocity from being counted
        {
...
;
    ...
    bool RunBodyTestMotion(Transform3D from, Vector3 motion, PhysicsTestMotionResult3D result = null)// bullshit code that does stuff required for body test motions
    {
        ...
        PhysicsTestMotionParameters3D param = new();
        param.From = from;
        param.Motion = motion;
