...
public partial class Door : InteractableObject
{
    ...
    void _func_godot_apply_properties(Dictionary Properties)
    {
        ...
        RotationSpeed = (float)Properties["rotationspeed"];
    ...
    public override void _Ready()
    {
        ...
        {
            GlobalPosition -= new Vector3(MeshInstance.GetAabb().Position.X, 0, MeshInstance.GetAabb().Position.Z + MeshInstance.GetAabb().Size.Z * 0.5f);
            Transform = Transform.Translated(new Vector3(MeshInstance.GetAabb().Position.X, 0, MeshInstance.GetAabb().Position.Z + MeshInstance.GetAabb().Size.Z * (-0.1f)));
        }
        else
        {
            GlobalPosition -= new Vector3(MeshInstance.GetAabb().Position.X, 0, MeshInstance.GetAabb().Position.Z);
            Transform = Transform.Translated(new Vector3(MeshInstance.GetAabb().Position.X + MeshInstance.GetAabb().Size.X * (-0.1f), 0, MeshInstance.GetAabb().Position.Z));
        }
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        if (!OnOff && !Stop)
        {
            ...
            RotationCounter += Mathf.RadToDeg(RotationSpeed * (float)delta);
        }
